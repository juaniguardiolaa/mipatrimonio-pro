...
export async function POST() {
  try {
    ...
    const result = await syncBrokerConnection(session.user.id, 'iol');
    ...
    if (error instanceof Error && error.message === 'BROKER_NOT_CONNECTED') {
      return jsonError('BROKER_NOT_CONNECTED', 'Debes conectar IOL primero', 404);
    }
    return jsonError('INTERNAL_SERVER_ERROR', 'No se pudo sincronizar IOL', 500);
  }
}
