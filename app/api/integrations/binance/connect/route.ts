...
export async function POST(request: Request) {
  try {
    ...
    const connection = await connectBroker(session.user.id, 'binance', payload.apiKey, payload.apiSecret);
    ...
    return jsonError('INTERNAL_SERVER_ERROR', 'No se pudo conectar Binance', 500);
  }
}
